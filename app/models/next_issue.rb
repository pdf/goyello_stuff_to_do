...
class NextIssue < ActiveRecord::Base
  ...
  # TODO: Rails bug
  #
  # ActiveRecord ignores :offset if :limit isn't added also.  But since we want
  # all the records, we need to provide a limit that will include everything
  #
  ...
  #
  def self.available(user, order, filter = { })
    order = "#{Issue.table_name}.created_on DESC" if order.nil?
  ...
  # Callback used to destroy all NextIssues when an issue is removed and send an
  # email if a user is below the What's Recommend threshold
  ...
  # +issue_ids+. New NextIssues will be created and if needed and old NextIssues
  # will be removed if they are unassigned.
  def self.reorder_list(user, issue_ids)
    ...
    removed_issues.each do |issue_id|
      ...
      logtime_entry = LogtimeEntry.find_by_user_id_and_issue_id(user.id, issue_id);
    ...
    issue_ids.each do |issue_id|
      if existing_list_position = next_issues_found.index(issue_id.to_i)
        ...
        # Have to resave next_issue since acts_as_list automatically moves it to
        # the bottom on create
