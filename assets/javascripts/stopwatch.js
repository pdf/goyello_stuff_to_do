// * Stopwatch class {{{
Stopwatch = function(listener, resolution, time, isPending) {
  this.startTime = 0;
  this.stopTime = 0;
  ...
  if(null != time[0] && !isNaN(parseInt(time[0])) && 
     null != time[1] && !isNaN(parseInt(time[1])) &&
     null != time[2] && !isNaN(parseInt(time[2])))
  {
    this.setElapsed(time[0], time[1], time[2]);
  }

  if(null != isPending && true == isPending)
  {
    this.start();
  }
}
